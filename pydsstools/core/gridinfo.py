...
class _GridInfo7(BaseModel):
    ...
    def update_albers_lower_left_cell_from_minxy(self):
        if self.grid_type == GridType.albers or self.grid_type == GridType.albers_time:
            logging.info(
                "Updating lower left cell indices using mix_xy of Albers GridInfo"
            )
    ...
    def update_albers_lower_left_cell_from_transform(self, transform):
        ...
        if abs(dx - dx_t) > 0.001:
            logging.error(
                "Cell size in gridinfo is different than cell size in the transform"
            )
        ...
        if self.grid_type == GridType.albers or self.grid_type == GridType.albers_time:
            ...
            if x_cell0 != 0.0 or y_cell0 != 0.0:
                logging.warning(
                    "Normally, the grid origin coordinates in SHG/Albers (epsg 5070) grid is (0,0). This is not the case here."
                )
    ...
    def update_specified_coords_cell0_from_transform(self, transform):
        if (
            ...
            if abs(dx - dx_t) > 0.001:
                ...
                logging.error(
                    "Skipping computation of coordinates of cell0 of grid located at bottom left corner"
                )
                return
...
class GridInfo(_GridInfo7):
    grid_type: Annotated[
        Literal[GridType.undefined_time, GridType.undefined], GridTypeField
    ] = GridType.undefined_time
    data_units: str = Field(
        default="", validation_alias=AliasChoices("data_units", "du", "data_unit")
    )
...
class HrapInfo(GridInfo):
    grid_type: Annotated[Literal[GridType.hrap_time, GridType.hrap], GridTypeField] = (
        GridType.hrap_time
    )
...
class AlbersInfo(GridInfo):
    grid_type: Annotated[
        Literal[GridType.albers_time, GridType.albers], GridTypeField
    ] = GridType.albers_time
    ...
    lat_1: float = Field(
        default=29.5,
        validation_alias=AliasChoices(
            "lat_1", "par1", "parallel1", "lat_1", "lat1", "first_parallel"
        ),
    )
    lat_2: float = Field(
        default=49.5,
    ...
    lon_0: float = Field(
        default=-96.0,
        validation_alias=AliasChoices(
            "lon_0", "cm", "cmer", "lon_0", "lon0", "central_meridian"
        ),
    )
    x_0: float = Field(
        alias="fe",
...
class SpecifiedInfo(GridInfo):
    grid_type: Annotated[
        Literal[GridType.specified_time, GridType.specified], GridTypeField
    ] = GridType.specified_time
    crs: str = Field(
        default="",
        validation_alias=AliasChoices(
    ...
    crs_name: str = Field(
        default="", validation_alias=AliasChoices("crs_name", "crsname", "srsname")
    )
    nodata: float = Field(
        allow_inf_nan=True,
...
def create_gridinfo(**kwargs) -> _GridInfo7:
    if not kwargs:
        raise TypeError(
            "Provide keyword arguments to create GridInfo (e.g., grid_type=GridType.hrap, ...)"
        )
